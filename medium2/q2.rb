...
# We simply took the contents of the block from the previous practice problem and moved it to a method, to which we are passing all of our outer variables.
...
# those outside, Ruby is simply re-using the objects, as these new variables
...
# values of our "outers", Ruby uses new objects for these variables to deal
# with their new values.

# One important difference to note is that before, we saw Ruby re-using
# the "42" object and just giving it a new value inside the block.
# Why the difference?
...
# variables inside the method call, we still have the same values and the
# same object id's down here AFTER the method call as we had before it!

# This is because our method call accepts VALUES as parameters. The names
# we give to those values in the definition of our method are SEPARATE from
# any other use of those same names.

# We used the same names there for convenience (and admittedly to build
# some suspense and allow us to clarify this point). We could just as easily have called the first parameter of our method definition a_Fred instead of a_outer.
